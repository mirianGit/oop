	...
	<div id="container">
		...
		<div
			...
			<blockquote>
				...
				%>
				<%
					String authorName = User.getUserById(curr_dish.getAuthorId())
							.getName();
				%>
		...
		<%
			if (request.getSession().getAttribute("signed") != null) {
				User curr_user = User.getUserById(User
						.getIdByName((String) request.getSession()
								.getAttribute("name")));
				if (!curr_dish.isRatedBy(curr_user)) {
		%>
		<form action="RateServlet" method="post">
			<input type="hidden" name="user_id" value=<%=curr_user.getId()%>>
			<input type="hidden" name="dish_id" value=<%=curr_dish.getId()%>>
			Rate It: <select name="rate">
				<option>1</option>
				<option>2</option>
				<option>3</option>
				<option>4</option>
				...
				<option>9</option>
				<option>10</option>
			</select> <input type="submit" value="sumbit">
		</form>
		<%
			} else {
		%>
		Rated: <select>
			<option><%=curr_dish.getRateBy(curr_user)%></option>
		</select>
		<%
			}
			}
		...
		<div style="margin-top: 20px">
			<a href="EditDish" class="button"> Edit Recipe</a>
		</div>
		<%
			}
		%>
